              ...
              <div class="card-body box-profile">
                ...
                <ul class="list-group list-group-unbordered mb-3">
                  <?php
                    ...
                    $sqlchart = "SELECT *,SUM(c.COT_PRICE) AS MonthRevenue, DATE_FORMAT(a.BOOK_DATE, '%Y')as YearsRevenue FROM tbl_appointment a LEFT JOIN tbl_cottage c ON a.COT_ID=c.COT_ID WHERE a.APP_STATUS=1 or a.APP_STATUS=2 GROUP BY YearsRevenue DESC"  ;
                    $cquery=$conn->query($sqlchart);
                    if($cquery->num_rows >0){
                      while($rowchart=$cquery->fetch_assoc()){
                        $TotalProgram=$rowchart['MonthRevenue'];
                        echo '<li class="list-group-item">
                          '.$rowchart['YearsRevenue'].'<a class="float-right"> <span class="badge badge-primary">'.$TotalProgram.'</span></a>
                          </li>';
                      }
                    }else{
                        echo '<li class="list-group-item">
            ...
            <div class="card">
              ...
              <?php 
                error_reporting(0);
                  ...
                  $sql = "SELECT *,SUM(c.COT_PRICE) AS TotalRevenue, DATE_FORMAT(a.BOOK_DATE, '%m')as BookMonth,DATE_FORMAT(a.BOOK_DATE, '%Y')as BookYear FROM tbl_appointment a LEFT JOIN tbl_cottage c ON a.COT_ID=c.COT_ID WHERE a.APP_STATUS=1 or a.APP_STATUS=2 GROUP BY MONTH(BookMonth)";
                  $query = $db->prepare($sql); 
                  $query->execute();
                  $fetch = $query->fetchAll();
                  foreach ($fetch as $key => $value) {
                    $female[] = array('Month'=>date('F',$value['BookMonth']), 'Revenue'=>$value['TotalRevenue']);
                  }
