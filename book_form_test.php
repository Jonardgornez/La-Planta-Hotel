                ...
                <div class="row g-5"> 
				  <div class="col-xl-6 wow fadeInUp" data-wow-delay="0.4s">
                        <div class="bg-light p-5 rounded h-100">
                            <h4 class="text-primary mb-4">Reservation Form</h4>
							<p>REMINDER: It is required to bring the orignal copy of your uploaded ID for validation</p>
							 ...
							 <input type="hidden" name="csrftoken" value="<?php echo htmlentities($_SESSION['token']); ?>" />
							   ...
							   <div class="col-lg-12">
								<div class="form-group">
								  <input type="hidden" class="form-control cotidss" value="<?=$get_cotid;?>" name="COT_ID" required>
                    ...
                    <div class="col-12 col-xl-6 wow fadeInUp rounded" data-wow-delay="0.2s" style="border:1px solid #ccc">
                        <div>
                            <div class="pb-1">
                                <p class="mb-0">PLEASE SELECT COTTAGE</p>
                            </div>
                            <div class="" >
										 <select class="form-control select2" id="office_select" class="form-control" onblur="myFunction();" autofocus required>
										  <?php
											$sql = "SELECT * FROM tbl_cottage WHERE COT_ID='$get_cotid'";
											$query = $conn->query($sql);
											while($row = $query->fetch_assoc()){
										  ?>
										  <option data-ccot="<?=$row['COT_ID'];?>" value="<?=$row['COT_ID'];?>" selected>[PRICE: <?=$row['COT_PRICE'];?>]-[GUEST: <?=$row['COT_NUM_GUEST'];?>] <?=$row['COT_NAME'];?></option>
										  <?php } ?>
									  </select>
...
<?php include "book_form_test_script.php";?>
<script>
